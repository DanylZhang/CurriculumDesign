...
namespace Danyl.SnnuURP.BLL
{
	...
	{
		private readonly Danyl.SnnuURP.DAL.TrainPlan dal=new Danyl.SnnuURP.DAL.TrainPlan();
		public TrainPlan()
		{}
		#region  BasicMethod
		...
		{
			return dal.Exists(MajorId,PlanId);
		}
		...
		public bool Delete(int MajorId,int PlanId)
		{
			...
			return dal.Delete(MajorId,PlanId);
		}
		/// <summary>
		/// 删除一条数据
		/// </summary>
		...
		{
			return dal.DeleteList(Danyl.SnnuURP.Common.PageValidate.SafeLongFilter(PlanIdlist,0) );
		}
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Danyl.SnnuURP.Model.TrainPlan GetModelByCache(int PlanId)
		{
			
			string CacheKey = "TrainPlanModel-" + PlanId;
			...
			return (Danyl.SnnuURP.Model.TrainPlan)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Danyl.SnnuURP.Model.TrainPlan> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Danyl.SnnuURP.Model.TrainPlan> DataTableToList(DataTable dt)
		{
			List<Danyl.SnnuURP.Model.TrainPlan> modelList = new List<Danyl.SnnuURP.Model.TrainPlan>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Danyl.SnnuURP.Model.TrainPlan model;
